...
internal class Plane : Geometry
{
    ...
    public Vector3D Normal
    {
        ...
        set
        {
            m_Normal = Vector3D.Normalize(value);
        }
    }
    ...
    override public bool Intersects(Ray ray, ref Vector3D intPoint)
    {
        ...
        if (Vector3D.Dot(m_Normal, D) != 0)
        {
            double t = Vector3D.Dot(m_Normal, m_Point - E) / Vector3D.Dot(m_Normal, D);
            if (t >= 0)
            {
                intPoint = E + (D * t);
                return true;
...
internal class Square : Plane
{
    ...
    public override bool Intersects(Ray ray, ref Vector3D intPoint)
    {
        ...
        if (Vector3D.Dot(Normal, D) != 0)
        {
            double t = Vector3D.Dot(Normal, Point - E) / Vector3D.Dot(Normal, D);
            if (t >= 0)
            {
                intPoint = E + (D * t);
